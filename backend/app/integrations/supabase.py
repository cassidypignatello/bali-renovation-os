...
# TODO: Update calling code and remove these aliases

save_estimate = save_project
get_estimate = get_project
update_estimate_status = update_project_status
save_transaction = save_payment
update_transaction_status = update_payment_status


async def get_material_price_history(material_name: str, limit: int = 10) -> list[dict]:
    """
    DEPRECATED: Use search_materials() instead.

    Get materials matching a name query with their embedded prices.
    """
    return await search_materials(material_name, limit)


async def save_material_price(price_data: dict) -> None:
    """
    DEPRECATED: Use update_material_prices() instead.

    This function is a no-op as prices are now embedded in materials table.
    """
    pass
...
async def get_workers_for_project(project_type: str, limit: int = 20) -> list[dict]:
    """
    DEPRECATED: Use get_workers_by_specialization() instead.
    """
    return await get_workers_by_specialization(project_type, limit=limit)
